...
dd.innerHTML='<button class="avmi" data-a="bg"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 14l5-4 5 5 3-2 5 4"/></svg>Background</button>'
 +'<button class="avmi" data-a="tips"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="12" cy="12" r="9"/><path d="M12 16v-4M12 8h.01"/></svg>Nav tooltips <span class="avtg" id="tipstate"></span></button>';
...
/* ---------- glass nav tooltips (setting: on/off for regular users) ---------- */
(function(){
var on=localStorage.getItem('sturij.navtips')!=='off';
var row=dd.querySelector('[data-a="tips"]'),lbl=dd.querySelector('#tipstate');
function paint(){lbl.textContent=on?'On':'Off';lbl.classList.toggle('on',on);}
paint();
row.onclick=function(e){e.stopPropagation();on=!on;localStorage.setItem('sturij.navtips',on?'on':'off');paint();};
var tip=null,tt=null;
function hide(){clearTimeout(tt);tt=null;if(tip){var t=tip;tip=null;t.remove();}}
document.addEventListener('pointerover',function(e){
  var b=e.target.closest?e.target.closest('.bobtile,.catbtn,.hbtn,.avbtn,.dockchip'):null;
  if(!b){hide();return;}
  if(!on){if(b._tt===undefined){b._tt=b.getAttribute('title');}return;}
  var text=b.getAttribute('title')||b._tt||(b.querySelector('b,span')||{}).textContent;
  if(b.getAttribute('title')){b._tt=b.getAttribute('title');b.removeAttribute('title');}
  if(!text)return;
  hide();
  tt=setTimeout(function(){
    tip=document.createElement('div');tip.className='glasstip';tip.textContent=text;
    document.body.appendChild(tip);
    var r=b.getBoundingClientRect(),tw=tip.offsetWidth;
    tip.style.left=Math.max(8,Math.min(innerWidth-tw-8,r.left+r.width/2-tw/2))+'px';
    tip.style.top=(r.bottom+9)+'px';
    if(window.Motion&&!document.body.classList.contains('rmotion')){try{Motion.animate(tip,{y:[-4,0],opacity:[0,1],scale:[.94,1]},{type:'spring',stiffness:420,damping:30});}catch(_){}}
  },260);
},true);
...
function openBgModal(){
  ...
  ov.querySelectorAll('.bgs').forEach(function(b){b.onclick=function(){
    ...
    $('world').style.setProperty('--board-bg',st.bg);document.documentElement.style.setProperty('--board-bg',st.bg);
...
/* ---------- restore ---------- */
if(st.bg){$('world').style.setProperty('--board-bg',st.bg);document.documentElement.style.setProperty('--board-bg',st.bg);}
