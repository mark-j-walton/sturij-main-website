...
(function(){
  ...
  window.openSearch=function(){var on=!draw.classList.contains('on');draw.classList.toggle('on',on);if(on){spring(draw,{transform:['translateY(-24px)','translateY(0px)']},{type:'spring',stiffness:320,damping:30});setTimeout(function(){el('sq').focus();},60);}};
  if(el('stab'))el('stab').onclick=window.openSearch;
  ...
  el('smic').onclick=function(){ if(!SR){toast('Voice not supported here — type instead');return;}
    if(rec){rec.stop();return;} rec=new SR();rec.lang='en-GB';rec.interimResults=false;el('smic').classList.add('rec');if(window.SturijRec)SturijRec.show('Voice search',12);
    rec.onresult=function(ev){el('sq').value=ev.results[0][0].transcript;go();};
    rec.onerror=function(){el('smic').classList.remove('rec');rec=null;if(window.SturijRec)SturijRec.hide();};
    rec.onend=function(){el('smic').classList.remove('rec');rec=null;if(window.SturijRec)SturijRec.hide();}; try{rec.start();}catch(_){} };
})();
...
(function(){
  ...
  window.openStudioTools=function(){buildGrid();buildMat();buildSwatchH();buildFloorH();el('modal').classList.add('on');};
  if(el('cog'))el('cog').onclick=window.openStudioTools;
...
function runCommand(t){
  ...
  if(/\b(open|show)\b.*\btool/.test(q)){if(window.openStudioTools)window.openStudioTools();return;}
...
(function(){
  var m=(el('vismenu')||document.createElement('div')); if(!m)return;
  ...
  document.addEventListener('pointerdown',function(e){ var vb=el('vis'); if(!m.contains(e.target)&&(!vb||!vb.contains(e.target))) m.classList.remove('on'); });
})();
if(el('vis'))el('vis').onclick=function(){ var m=(el('vismenu')||document.createElement('div')); m?m.classList.toggle('on'):runVisualise(); };
function runVisualise(){
  var b=el('vis'); if(b&&b.classList.contains('rec'))return;
  if(!paints[0]&&!sel.board){toast('Choose at least a wall colour or a board first');return;}
  if(b)b.classList.add('rec');SturijProgress.open('Rendering a '+visRoom.toLowerCase(),['Reading your pairings\u2026','Sending to the studio\u2026','Composing the room\u2026','Painting light and shadow\u2026','Final grade\u2026']);
  buildRenderRefs(function(refs){
    composeScene(function(cv){
      ...
      .catch(function(e){if(b)b.classList.remove('rec');SturijProgress.fail('Render failed \u2014 is this origin whitelisted on the visualiser?');});
    });
  });
};
...
function getPaints(cb){if(window.FB&&FB.length)cb(FB);else setTimeout(function(){getPaints(cb);},400);}
getPaints(function(paints){
  ...
  pd.addEventListener('pointerdown',function(e){
    ...
    try{addPaint([d.getAttribute('data-n'),d.getAttribute('data-h')]);if(window.toast)toast((typeof paintTarget!=='undefined'?paintTarget:'wall')+' \u00b7 '+d.getAttribute('data-n'));}catch(_){/**/}
...
/* centre trio toggle-group indicator (Photo · Snip · Talk) */
(function(){
  var tc=document.querySelector('.tc');if(!tc)return;
  var ind=document.createElement('div');ind.className='tcind';tc.insertBefore(ind,tc.firstChild);
  var idleT=null;
  [].forEach.call(tc.querySelectorAll('.hbtn'),function(b){
    b.addEventListener('click',function(){
      [].forEach.call(tc.querySelectorAll('.hbtn'),function(x){x.classList.remove('on');});
      b.classList.add('on');
      ind.style.left=b.offsetLeft+'px';ind.style.width=b.offsetWidth+'px';ind.classList.add('on');
      clearTimeout(idleT);idleT=setTimeout(function(){ind.classList.remove('on');b.classList.remove('on');},2600);
    });
  });
})();
