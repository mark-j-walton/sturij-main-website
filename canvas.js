...
/* mouse light + nav label mode (icons|text|both, persisted) */
(function(){
  var raf=0;
  document.addEventListener('pointermove',function(e){
    if(raf)return;raf=requestAnimationFrame(function(){raf=0;
      ...
      var g=document.querySelector('.bobgrid.in');
      if(g)g.querySelectorAll('.bobtile').forEach(function(t){
        var r=t.getBoundingClientRect();
        t.style.setProperty('--tx',r.left+'px');t.style.setProperty('--ty',r.top+'px');});
      document.querySelectorAll('.citem.swatch .fill').forEach(function(f){
        var r2=f.getBoundingClientRect();
        if(r2.bottom<0||r2.top>innerHeight||r2.right<0||r2.left>innerWidth)return;
        f.style.setProperty('--ix',r2.left+'px');f.style.setProperty('--iy',r2.top+'px');});});
  },{passive:true});
  var nm=localStorage.getItem('sturij.navmode')||'both';
  document.body.setAttribute('data-navmode',nm);
  window.setNavMode=function(m){document.body.setAttribute('data-navmode',m);localStorage.setItem('sturij.navmode',m);};
})();
var FONTS={serif:"'Source Serif 4',serif",sans:"'Helvetica Neue',system-ui,sans-serif",mono:"'IBM Plex Mono',monospace"};
var NOTECOLS=['#F6EEC9','#F7E9A8','#CFE3D4','#DCE3EE','#EFD9C8'];
/* layer bands: fx(1) < walls(2..) < wall-locked items(wall z+1) < loose items(zTop band) < furniture/pins(fTop band).
   Walls store z as negative (-9000..); loose items 1..; furniture 10000... An item locked onto a wall inherits the wall's level. */
function wallUnder(q){
  if(q.type==='wall')return null;
  var cx=q.x+q.w/2,cy=q.y+q.h/2,best=null;
  items.forEach(function(w){if(w.type!=='wall')return;
    if(cx<w.x||cx>w.x+w.w||cy<w.y||cy>w.y+w.h)return;
    if(!best||(w.z||0)>(best.z||0))best=w;});
  return best;
}
function zOf(it){
  if(it.type!=='wall'&&it.locked){var w=wallUnder(it);if(w)return (w.z||-9000)+1;}
  return it.z||1;
}
function restackWall(w){
  items.forEach(function(q){if(q.type==='wall'||!q.locked)return;
    var n=nodeFor(q);if(n&&wallUnder(q)===w)n.style.zIndex=zOf(q);});
}
/* minimised post-its — loose stack above the chat fab */
function renderMinNotes(){
  var mins=items.filter(function(q){return q.type==='note'&&q.noteMin;});
  var dk=document.getElementById('notedock');
  if(!mins.length){if(dk)dk.remove();return;}
  if(!dk){dk=document.createElement('div');dk.id='notedock';document.body.appendChild(dk);}
  dk.innerHTML='';
  mins.forEach(function(q,i){
    var m=document.createElement('div');m.className='mn';
    m.style.background=q.noteHex||'#F6EEC9';
    m.style.transform='rotate('+(((i*47)%9)-4)+'deg) translateX('+(((i*31)%11)-5)+'px)';
    m.textContent=q.tagName||q.text||'Note';m.title='Bring back to the board';
    m.onclick=function(){q.noteMin=false;render(q);settle(nodeFor(q),{scale:[.7,1],opacity:[0,1]});renderMinNotes();qsave();};
    dk.appendChild(m);settle(m,{scale:[.6,1],opacity:[0,1]},{delay:i*.04});
  });
}
/* anchor a dropped note to the topmost card under its centre; swatches also tag it */
function anchorNote(it){
  var cx=it.x+it.w/2,cy=it.y+it.h/2,host=null;
  items.forEach(function(q){
    if(q===it||q.type==='note'||q.noteMin||q.docked||q.dockedTo||q.hiddenPage)return;
    if(cx<q.x||cx>q.x+q.w||cy<q.y||cy>q.y+q.h)return;
    if(!host||(q.z||0)>(host.z||0))host=q;});
  var was=it.anchor;
  it.anchor=host?host.id:null;
  if(host&&(host.type==='swatch'||host.type==='stack')){
    var c=host.type==='stack'?(host.cards&&host.cards[host.ix||0]):host;
    if(c){it.tagBrand='Sturij';it.tagName=c.name||c.hex||'Swatch';it.tagHex=c.hex||null;it.tagMeta=c.hex?c.hex.toUpperCase():'Material';rerender(it);toast('Tagged \u00b7 '+it.tagName);}}
  else if(host&&!was)toast('Anchored to '+(host.name||host.type));
}
var hist=[],redoS=[];
...
/* ===== shelf docks — swatch containers (system or user-made) ===== */
function shelfSize(it){var n=Math.max(1,(it.cards||[]).length);
  if(it.min){it.w=Math.max(210,(it.name||'').length*9+120);it.h=46;}
  else{it.w=32+n*92+(n-1)*12;it.h=232;}}
function shelfHTML(it){
  var ink=it.frameHex&&inkFor(it.frameHex)==='light'?' inklight':'';
  var bg=it.frameHex?' style="background:'+it.frameHex+'"':'';
  if(it.min)return '<div class="inner shelfcard min'+ink+'"'+bg+'><div class="shmin"><span class="shnm2">'+(it.name||'Shelf')+'</span><button class="sharr" title="Open"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="12" height="12"><path d="M6 9l6 6 6-6"/></svg></button></div></div>';
  var h='<div class="inner shelfcard'+ink+'"'+bg+'><div class="shhd"></div><div class="shrow">';
  (it.cards||[]).forEach(function(c,i){h+='<div class="shtile" data-si="'+i+'"><div class="shfill" style="'+cardFill(c)+'"></div></div>';});
  h+='</div><div class="shft"><span class="shnm"'+(it.userMade?' contenteditable="true" spellcheck="false"':'')+'>'+(it.name||(it.userMade?'Custom swatches':'Suggested pairings'))+'</span></div></div>';
  return h;}
function wireShelf(d,it){
  var nm=d.querySelector('.shnm');
  if(nm&&it.userMade){nm.addEventListener('pointerdown',function(e){e.stopPropagation();});
    nm.addEventListener('blur',function(){it.name=nm.textContent.trim()||'Custom swatches';qsave();});
    nm.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();nm.blur();}});}
  var arr=d.querySelector('.sharr');
  if(arr){arr.addEventListener('pointerdown',function(e){e.stopPropagation();});
    arr.addEventListener('click',function(e){e.stopPropagation();it.min=false;shelfSize(it);rerender(it);qsave();});}
  d.querySelectorAll('.shtile').forEach(function(t){
    t.addEventListener('pointerdown',function(e){
      e.stopPropagation();e.preventDefault();
      var si=+t.getAttribute('data-si'),moved=false,gh=null,sx=e.clientX,sy=e.clientY;
      function mv(ev){
        if(!moved&&Math.hypot(ev.clientX-sx,ev.clientY-sy)<6)return;
        moved=true;
        if(!gh){gh=t.cloneNode(true);gh.classList.add('shghost');document.body.appendChild(gh);}
        gh.style.left=(ev.clientX-46)+'px';gh.style.top=(ev.clientY-66)+'px';
      }
      function up(ev){document.removeEventListener('pointermove',mv);document.removeEventListener('pointerup',up);
        ...
        pushHist();
        var c=it.cards.splice(si,1)[0];
        var wr=world.getBoundingClientRect();
        var wx=(ev.clientX-wr.left-view.x)/view.z,wy=(ev.clientY-wr.top-view.y)/view.z;
        var nw={type:'swatch',hex:c.hex,src:c.src,name:c.name,x:wx-46,y:wy-60,w:92,h:120,id:'c'+Date.now()+Math.random().toString(36).slice(2,6),z:++zTop};
        items.push(nw);render(nw);settle(nodeFor(nw),{scale:[.85,1],opacity:[0,1]});
        if(!it.cards.length){items.splice(items.indexOf(it),1);var n0=nodeFor(it);if(n0)n0.remove();toast('Shelf empty \u2014 closed');}
        else{shelfSize(it);rerender(it);}
        qsave();
      }
...
function shelfTargetAt(it){
  var cx=it.x+it.w/2,cy=it.y+it.h/2,best=null;
  items.forEach(function(q){if(q.type!=='shelf'||q.min||q.hiddenPage)return;
    if(cx<q.x||cx>q.x+q.w||cy<q.y||cy>q.y+q.h)return;
    if(!best||(q.z||0)>(best.z||0))best=q;});
  return best;}
function mergeIntoShelf(drag,target){
  pushHist();target.cards.push(asCard(drag));
  items.splice(items.indexOf(drag),1);var dn=nodeFor(drag);if(dn)dn.remove();
  var _sn=null;
  shelfSize(target);rerender(target);qsave();
  _sn=nodeFor(target);if(_sn){settle(_sn.querySelector('.shrow'),{scale:[1.03,1]});var _lt=_sn.querySelectorAll('.shtile');if(_lt.length)settle(_lt[_lt.length-1],{scale:[.7,1],opacity:[0,1]});}
  toast((target.name||'Shelf')+' \u00b7 '+target.cards.length+' swatches');}
function makeShelf(g){
  var sws=g.filter(function(q){return q.type==='swatch';});if(sws.length<2)return;
  pushHist();
  var x=Math.min.apply(null,sws.map(function(q){return q.x;})),y=Math.min.apply(null,sws.map(function(q){return q.y;}));
  sws.sort(function(a,b){return a.x-b.x;});
  var it={type:'shelf',name:'Custom swatches',userMade:true,cards:sws.map(asCard),x:x,y:y,
    id:'c'+Date.now()+Math.random().toString(36).slice(2,6),z:++zTop};
  sws.forEach(function(q){items.splice(items.indexOf(q),1);var n=nodeFor(q);if(n)n.remove();});
  shelfSize(it);items.push(it);render(it);qsave();updateCtx();
  toast('Shelf \u2014 drag swatches on or off, click the title to rename');}
...
/* Motion spring helpers — all placement/exit movement runs through these (DESIGN.md §2.4: things settle) */
function settle(el,kf,opts){if(!el||!window.Motion||document.body.classList.contains('rmotion'))return null;
  try{return Motion.animate(el,kf,Object.assign({type:'spring',stiffness:340,damping:26},opts||{}));}catch(_){return null;}}
function exitThen(el,kf,done){
  if(!el||!window.Motion||document.body.classList.contains('rmotion')){done();return;}
  try{var a=Motion.animate(el,kf,{duration:.18,ease:[.22,1,.36,1]});a.then(done,done);}catch(_){done();}}
...
function render(it){
  ...
  if(it.noteMin){renderMinNotes();return null;}
  var d=document.createElement('div');d.className='citem '+(it.type==='note'?'bnote':it.type)+(it.locked?' locked':'');d.dataset.id=it.id;
  d.style.cssText='left:'+it.x+'px;top:'+it.y+'px;width:'+it.w+'px;height:'+it.h+'px;z-index:'+zOf(it);
  ...
  else if(it.type==='shelf')d.innerHTML=shelfHTML(it);
  ...
  else if(it.type==='note'){var nc=it.noteHex||'#F6EEC9';d.style.setProperty('--cardcol',nc);
    d.innerHTML='<div class="inner" style="background-color:'+nc+'">'
      +(it.tagName?'<div class="ntag"><span class="nbr">'+(it.tagBrand||'Sturij')+'</span><span class="nnm">'+(it.tagHex?'<span class="ndot" style="background:'+it.tagHex+'"></span>':'')+it.tagName+'</span>'+(it.tagMeta?'<span class="nmeta">'+it.tagMeta+'</span>':'')+'</div>':'')
      +'<div class="body" contenteditable="true"></div>'
      +'<div class="nedge" title="Note colour"></div><div class="ncolors">'+NOTECOLS.map(function(c){return '<button data-nc="'+c+'" class="'+(c===nc?'on':'')+'" style="background:'+c+'"></button>';}).join('')+'</div></div>';}
  ...
  d.innerHTML+='<button class="cx" title="'+((it.type==='swatch'||it.type==='img')?'Remove from canvas':'Remove')+'" aria-label="Remove"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" width="11" height="11"><path d="M6 6l12 12M18 6L6 18"/></svg></button>'
    +((it.type==='swatch'||it.type==='img'||it.type==='shelf'||it.type==='note')?'<button class="cm" title="'+(it.type==='shelf'?'Minimise':it.type==='note'?'Tuck under chat':'Return to home')+'" aria-label="Minimise"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" width="11" height="11"><path d="M5 12h14"/></svg></button>':'')
    +'<span class="rs" title="Resize"></span>'
    +((it.type==='swatch'||it.type==='img')?'<button class="rh" title="Rotate · drag (shift snaps)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" width="11" height="11"><path d="M21 12a9 9 0 1 1-3-6.7"/><path d="M21 3v4h-4"/></svg></button>':'');
  if(it.frameHex)d.style.setProperty('--cardcol',it.frameHex);
  if(it.rot){var _in0=d.querySelector('.inner');if(_in0)_in0.style.transform='rotate('+it.rot+'deg)';}
  var _rh=d.querySelector('.rh');
  if(_rh)_rh.addEventListener('pointerdown',function(e){
    e.stopPropagation();e.preventDefault();pushHist();
    var r=d.getBoundingClientRect(),cx0=r.left+r.width/2,cy0=r.top+r.height/2;
    var a0=Math.atan2(e.clientY-cy0,e.clientX-cx0)*180/Math.PI-(it.rot||0);
    function mv(ev){var a=Math.atan2(ev.clientY-cy0,ev.clientX-cx0)*180/Math.PI-a0;
      if(ev.shiftKey)a=Math.round(a/15)*15;
      it.rot=Math.round(a);var inn=d.querySelector('.inner');if(inn)inn.style.transform='rotate('+it.rot+'deg)';}
    function up(){document.removeEventListener('pointermove',mv);document.removeEventListener('pointerup',up);qsave();}
    document.addEventListener('pointermove',mv);document.addEventListener('pointerup',up);
  });
  if(it.type==='img'){var _im=d.querySelector('img');if(_im)_im.onload=function(){it.natW=_im.naturalWidth;};}
  if(it.type==='note'||it.type==='text'){var b=d.querySelector('.body');b.textContent=it.text||'';
    /* drag anywhere; a plain click (no drag) drops you straight into typing */
  ...
  if(it.type==='note'){
    var ne=d.querySelector('.nedge');
    ne.addEventListener('pointerdown',function(e){e.stopPropagation();});
    ne.addEventListener('click',function(e){e.stopPropagation();d.classList.toggle('colopen');});
    d.querySelectorAll('.ncolors button').forEach(function(cb){
      cb.addEventListener('pointerdown',function(e){e.stopPropagation();});
      cb.addEventListener('click',function(e){e.stopPropagation();it.noteHex=cb.getAttribute('data-nc');rerender(it);qsave();});});}
  d.querySelector('.cx').addEventListener('click',function(e){e.stopPropagation();if(it.locked)return;pushHist();items=items.filter(function(q){return q!==it;});updateCtx();qsave();exitThen(d,{scale:.92,opacity:0,y:10},function(){d.remove();});});
  var _cm=d.querySelector('.cm');
  if(_cm)_cm.addEventListener('click',function(e){e.stopPropagation();if(it.locked)return;
    if(it.type==='shelf'){it.min=!it.min;shelfSize(it);rerender(it);qsave();return;}
    if(it.type==='note'){it.noteMin=true;updateCtx();qsave();exitThen(d,{scale:.7,opacity:0,x:60,y:80},function(){d.remove();renderMinNotes();});return;}
    pushHist();items=items.filter(function(q){return q!==it;});updateCtx();qsave();
    exitThen(d,{scale:.92,opacity:0,y:-10},function(){d.remove();});
    toast(it.type==='img'?'Returned to library':'Returned to the drawer');});
  /* drag: selection model (shift adds), group move, lock respected */
  d.addEventListener('pointerdown',function(e){
    if(e.target.closest('.cx,.cm'))return;
    e.stopPropagation();
    ...
    if(it.type==='wall'){it.z=(window._wTop=(window._wTop||-9000)+1);d.style.zIndex=it.z;restackWall(it);}
    ...
    /* anchored notes ride with their host */
    items.forEach(function(q){if(q.type==='note'&&q.anchor&&!q.noteMin&&group.indexOf(q)<0&&group.some(function(g){return g.id===q.anchor;}))starts.push({q:q,x:q.x,y:q.y});});
    var sx=e.clientX,sy=e.clientY,ow=it.w,oh=it.h,oy=it.y,moved=false,rsRaf=0;
    ...
    function mv(ev){
      ...
      if(rs){
        if(it.type==='wall'){
          ...
          /* everything else resizes proportionally from its corner; swatch/photo use the top-right corner (bottom edge anchored); photos stop before they degrade */
          var flip=(it.type==='swatch'||it.type==='img');
          var k=Math.max((ow+dx)/ow,(oh+(flip?-dy:dy))/oh);
          var maxK=(it.type==='img')?Math.max(1,((it.natW||1800)*1.2)/ow):(it.type==='swatch'&&it.src?5:99);
          k=Math.min(k,maxK);
          it.w=Math.max(70,ow*k);it.h=Math.max(54,oh*k);
          if(flip){it.y=oy+(oh-it.h);}
        }
        if(rsRaf)return;rsRaf=requestAnimationFrame(function(){rsRaf=0;
          d.style.width=it.w+'px';d.style.height=it.h+'px';d.style.top=it.y+'px';
    ...
    function up(){document.removeEventListener('pointermove',mv);document.removeEventListener('pointerup',up);
      if(moved){hist.push(pre);if(hist.length>60)hist.shift();redoS.length=0;qsave();
        if(!rs&&it.type==='swatch'&&group.length===1){var sh1=shelfTargetAt(it);if(sh1)mergeIntoShelf(it,sh1);else{var tgt=stackTargetAt(it);if(tgt)mergeIntoStack(it,tgt);}}
        if(!rs&&it.type==='note'&&group.length===1)anchorNote(it);
      }
      else if(it.type==='note'||it.type==='text'){var b3=d.querySelector('.body');if(b3&&document.activeElement!==b3&&!window.getSelection().toString())b3.focus();}}
    document.addEventListener('pointermove',mv);document.addEventListener('pointerup',up);
    e.preventDefault();
  });
  if(it.type==='plan'){wirePlan(d,it);applyCardGrid(d,it);}
  if(it.type==='stack')wireStack(d,it);
  if(it.type==='shelf')wireShelf(d,it);
...
function add(it){pushHist();it.id=it.id||('c'+Date.now()+Math.random().toString(36).slice(2,6));it.z=(it.type==='wall')?(window._wTop=(window._wTop||-9000)+1):((it.type==='furn'||it.type==='pin')?++fTop:++zTop);items.push(it);
  ...
  render(it);settle(nodeFor(it),{scale:[.94,1],opacity:[0,1]});qsave();return it;}
...
/* ---------- glass context menu (right-click) — related tools at the cursor ---------- */
(function(){
var GM_LABELS={front:'Bring forward',back:'Send back',lock:'Lock / unlock',rot:'Rotate 90\u00b0',frame:'Card colour',pal:'Palette',shelf:'Make a shelf',unstk:'Unstack',fdim:'Size\u2026',dock:'Dock',del:'Delete'};
var gm=null;
function closeGM(){if(gm){var g=gm;gm=null;g.remove();}}
document.addEventListener('pointerdown',function(e){if(gm&&!gm.contains(e.target))closeGM();},true);
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeGM();});
world.addEventListener('contextmenu',function(e){
  var d=e.target.closest('.citem');if(!d)return;
  e.preventDefault();closeGM();
  if(!d.classList.contains('sel'))d.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,clientX:e.clientX,clientY:e.clientY}));
  updateCtx();
  gm=document.createElement('div');gm.className='glassmenu';
  var rows=0;
  ctx.querySelectorAll('[data-a]').forEach(function(b){
    var a=b.getAttribute('data-a');
    if(!GM_LABELS[a]||b.style.display==='none')return;
    if(a==='del'&&rows){var s=document.createElement('div');s.className='gsep';gm.appendChild(s);}
    var r=document.createElement('button');
    if(a==='del')r.className='gdanger';
    r.innerHTML='<span class="gdot"></span>'+GM_LABELS[a];
    r.onclick=function(){closeGM();b.click();};
    gm.appendChild(r);rows++;
  });
  if(!rows){closeGM();return;}
  gm.style.left=Math.min(e.clientX,innerWidth-200)+'px';
  gm.style.top=Math.min(e.clientY,innerHeight-gm.childElementCount*36-20)+'px';
  document.body.appendChild(gm);
  if(window.Motion&&!document.body.classList.contains('rmotion')){try{Motion.animate(gm,{scale:[.9,1],opacity:[0,1],y:[-4,0]},{type:'spring',stiffness:420,damping:30});}catch(_){/**/}}
});
})();
/* ---------- context bar ---------- */
var ctx=el('ctxbar');
...
function updateCtx(){
  ...
  var swsAll=g.filter(function(q){return q.type==='swatch';});
  ctx.querySelector('[data-a="shelf"]').style.display=(swsAll.length===g.length&&swsAll.length>=2)?'':'none';
  ctx.querySelector('[data-a="saver"]').style.display=(g.length>=1&&g.length<=3&&g.every(function(q){return q.type==='img'&&q.src;}))?'':'none';
  ctx.querySelector('[data-a="rot"]').style.display=(g.length&&g.every(function(q){return q.type==='furn'||q.type==='swatch'||q.type==='img';}))?'':'none';
  ...
  ctx.querySelector('[data-a="frame"]').style.display=(g.length===1&&(g[0].type==='wall'||g[0].type==='plan'||g[0].type==='shelf'))?'':'none';
  ...
  if(a==='lock'){pushHist();var to=!g.every(function(q){return q.locked;});
    ...
    g.forEach(function(q){var n=nodeFor(q);
      if(q.type==='wall'){
        ...
        if(n){n.style.zIndex=q.z;}restackWall(q);
        return;
...
/* ---------- persistent paint disc — vanilla port of the Motion+ ColorPicker ---------- */
(function(){
function applyPaint(hex,nm){
  var g=selItems(),t=g.length===1?g[0]:null;
  pushHist();
  if(!t){boardBg=hex;applyBg();qsave();toast('Canvas \u2014 '+nm);return;}
  var n=nodeFor(t);
  if(t.type==='wall'){t.hex=hex;if(n){var f=n.querySelector('.wface');if(f)f.style.background=hex;}}
  else if(t.type==='room'){t.wallHex=hex;if(n)n.querySelector('.rvface').innerHTML=roomSVG(t);}
  else if(t.type==='note'){t.noteHex=hex;rerender(t);}
  else if(t.type==='plan'||t.type==='shelf'){t.frameHex=hex;if(n)n.style.setProperty('--cardcol',hex);
    if(t.type==='shelf')rerender(t);
    else if(n){applyCardGrid(n,t);var fc=n.querySelector('.plancard');if(fc){fc.style.background=hex;fc.classList.toggle('inklight',inkFor(hex)==='light');}}}
  else{t.frameHex=hex;if(n)n.style.setProperty('--cardcol',hex);}
  qsave();toast(nm+' \u2014 applied');
}
loadPaints(function(paints){
  var pd=document.createElement('div');pd.id='paintdisc';document.body.appendChild(pd);
  var dots=[],pi=0,step=Math.max(1,Math.floor(paints.length/18));
  [[0,1],[20,6],[40,12]].forEach(function(rg){
    for(var i=0;i<rg[1];i++){
      var p=paints[Math.min(paints.length-1,pi*step)];pi++;
      var ang=(i/rg[1])*Math.PI*2-Math.PI/2;
      var d=document.createElement('button');d.className='pdot';
      d._bx=Math.cos(ang)*rg[0];d._by=Math.sin(ang)*rg[0];
      d._x=0;d._y=0;d._vx=0;d._vy=0;
      var hx=rg[0]===0?'#FDFCF8':p[1],nm=rg[0]===0?'Paper white':p[0];
      d.style.background=hx;if(rg[0]===0)d.classList.add('pcore');
      d.setAttribute('data-h',hx);d.setAttribute('data-n',nm);d.title=nm;
      d.style.transform='translate('+d._bx.toFixed(1)+'px,'+d._by.toFixed(1)+'px)';
      pd.appendChild(d);dots.push(d);
    }});
  var idleT=0;
  function poke(){pd.classList.remove('dorm');clearTimeout(idleT);idleT=setTimeout(function(){pd.classList.add('dorm');},60000);}
  poke();
  /* Motion example maths: push only when the pointer is inside the disc; max 80px falloff, ~5px magnitude, soft spring */
  var mx=-9999,my=-9999,running=false;
  function tick(){
    var r=pd.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2;
    var inside=Math.hypot(mx-cx,my-cy)<=112,still=true;
    dots.forEach(function(d){
      var tx=0,ty=0;
      if(inside){
        var ddx=(cx+d._bx)-mx,ddy=(cy+d._by)-my,dd=Math.hypot(ddx,ddy);
        if(dd<80&&dd>0.01){var s=(1-dd/80)*6;tx=ddx/dd*s;ty=ddy/dd*s;}
      }
      d._vx=(d._vx+(tx-d._x)*0.09)*0.82;d._vy=(d._vy+(ty-d._y)*0.09)*0.82;
      d._x+=d._vx;d._y+=d._vy;
      if(Math.abs(tx-d._x)>.05||Math.abs(d._vx)>.02||Math.abs(ty-d._y)>.05)still=false;
      d.style.transform='translate('+(d._bx+d._x).toFixed(2)+'px,'+(d._by+d._y).toFixed(2)+'px)';
    });
    if(still&&!pd.classList.contains('near')){running=false;return;}
    requestAnimationFrame(tick);
  }
  function ensureRun(){if(!running){running=true;requestAnimationFrame(tick);}}
  document.addEventListener('pointermove',function(e){
    mx=e.clientX;my=e.clientY;
    var r=pd.getBoundingClientRect();
    var near=Math.hypot(mx-(r.left+r.width/2),my-(r.top+r.height/2))<170;
    if(near){if(!pd.classList.contains('near')){pd.classList.add('near');poke();}ensureRun();}
    else pd.classList.remove('near');
  },{passive:true});
  pd.addEventListener('pointerdown',function(e){
    var d=e.target.closest('.pdot');if(!d)return;
    e.stopPropagation();e.preventDefault();poke();
    applyPaint(d.getAttribute('data-h'),d.getAttribute('data-n'));
    if(window.Motion&&!document.body.classList.contains('rmotion')){try{Motion.animate(d,{scale:[1.2,1]},{type:'spring',stiffness:200,damping:30});}catch(_){/**/}}
  });
  var _uc=updateCtx;updateCtx=function(){_uc();pd.classList.toggle('armed',selItems().length>0);};
});
})();
...
function openPalette(target){
  ...
  loadPaints(function(paints){
    ...
    wrap.addEventListener('pointerdown',function(e){
      ...
      if((target.type==='wall'||target.type==='plan'||target.type==='shelf')&&target._paintFrame){delete target._paintFrame;target.frameHex=hex;n=nodeFor(target);if(n)n.style.setProperty('--cardcol',hex);if(n&&target.type==='plan')applyCardGrid(n,target);if(target.type==='shelf'){rerender(target);}else if(n){var fc=n.querySelector('.wallcard,.plancard');if(fc){fc.style.background=hex;fc.classList.toggle('inklight',inkFor(hex)==='light');}}}
...
(function(){
  ...
  var custUnits=[];try{custUnits=JSON.parse(localStorage.getItem('sj-wardunits')||'[]');}catch(e){}
  custUnits=custUnits.filter(function(q){return q.colspec;});
  function custSave(){try{localStorage.setItem('sj-wardunits',JSON.stringify(custUnits));}catch(e){}}
  /* Carcass engine — ported from sturij-visualiser/src/carcass/{engine,cutlist}.ts.
     Interior derived from shell (innerW=W−2t, innerH=H−2t, innerD=D−backT), equal division,
     Blum reveals (top5/bottom0/left2/right2/between5), cut list → board m² → price. Units mm. */
  var REVEAL={top:5,bottom:0,left:2,right:2,between:5};
  var WRATES={standard:{board:22,hinge:4,runner:14,rail:12,joinDay:260},premium:{board:48,hinge:9,runner:28,rail:22,joinDay:320}};
  var COLPRESETS=[['door','Door · 2 shelves'],['hang2','Double hang'],['hang1','Single hang · shelf'],['drw3','Drawers × 3'],['drw4','Drawers × 4'],['shelf5','Shelves × 5'],['open','Open · no front']];
  function wardEngine(u){
    var t=18,backT=18,W=u.wmm,H=u.hmm,D=u.dmm,n=u.cols;
    var innerW=W-2*t,innerH=H-2*t,innerD=D-backT;
    var clear=(innerW-(n-1)*t)/n;                       /* equalDivision */
    var rows=[],hinges=0,runners=0,rails=0;
    function add(part,l,w){rows.push({part:part,l:l,w:w});}
    add('Left cabinet side',H,D);add('Right cabinet side',H,D);
    for(var d=0;d<n-1;d++)add('Central panel',innerH,innerD);
    add('Top cross member - front',innerW,80);add('Top cross member - rear',innerW,80);
    add('Cabinet base',innerW,innerD);
    add('Cabinet back',H,W);                            /* full applied back */
    var fh=H-REVEAL.top-REVEAL.bottom;                  /* frontHeight */
    var fw=(W-REVEAL.left-REVEAL.right-(n-1)*REVEAL.between)/n; /* frontWidths */
    var hingesPerDoor=H<=1200?2:H<=2000?3:4;
    u.colspec.forEach(function(cs){
      if(cs==='door'){add('Door front',fh,fw);hinges+=hingesPerDoor;add('Shelf',clear,innerD);add('Shelf',clear,innerD);}
      else if(cs==='hang1'){add('Door front',fh,fw);hinges+=hingesPerDoor;rails+=1;add('Shelf',clear,innerD);}
      else if(cs==='hang2'){add('Door front',fh,fw);hinges+=hingesPerDoor;rails+=2;}
      else if(cs==='drw3'||cs==='drw4'){
        var k=cs==='drw4'?4:3,eh=(fh-(k-1)*REVEAL.between)/k,bh=Math.max(60,eh-30);
        for(var i=0;i<k;i++){add('Drawer front',eh,fw);
          add('Drawer box side',innerD-20,bh);add('Drawer box side',innerD-20,bh);
          add('Drawer box back',clear-90,bh);add('Drawer box bottom',clear-90,innerD-40);runners+=1;}}
      else if(cs==='shelf5'){for(var s=0;s<5;s++)add('Shelf',clear,innerD);}
    });
    var area=0;rows.forEach(function(r){area+=r.l*r.w/1e6;});
    var R=WRATES[u.tier==='premium'?'premium':'standard'];
    var board=area*1.15*R.board;                        /* 15% nesting waste */
    var hw=hinges*R.hinge+runners*R.runner+rails*R.rail;
    var days=Math.max(0.5,Math.round(n*0.4*2)/2);
    var labour=days*R.joinDay;
    return {rows:rows,parts:rows.length,area:Math.round(area*100)/100,hinges:hinges,runners:runners,rails:rails,
      days:days,board:Math.round(board),hw:Math.round(hw),labour:Math.round(labour),price:Math.round(board+hw+labour)};
  }
  function openBuilder(edit){
    var u=edit||{name:'',wmm:1200,hmm:2200,dmm:600,cols:2,colspec:['hang2','door'],tier:'standard'};
    var ov=document.createElement('div');ov.className='trmodal';
    function row(l,c){return '<div class="ifrow"><span>'+l+'</span>'+c+'</div>';}
    function num(id,v,st){return '<input class="ifuc" id="'+id+'" type="number" step="'+(st||50)+'" value="'+v+'" style="width:70px">';}
    function draw(){
      while(u.colspec.length<u.cols)u.colspec.push('door');u.colspec.length=u.cols;
      var e=wardEngine(u);u.price=e.price;
      ov.innerHTML='<div class="trbox infobox"><div class="ifhd"><span class="oh">Build a wardrobe unit · carcass engine</span><button class="ifx">×</button></div><div class="ifbody">'
        +row('Width mm',num('bw',u.wmm))+row('Height mm',num('bh',u.hmm))+row('Depth mm',num('bd',u.dmm))
        +row('Columns','<span class="wgc bctr" data-c="cols"><button data-d="-1">−</button><b>'+u.cols+'</b><button data-d="1">+</button></span>')
        +u.colspec.map(function(cs,i){return row('Bay '+String.fromCharCode(65+i),
          '<select class="ifsel" data-bay="'+i+'">'+COLPRESETS.map(function(p){return '<option value="'+p[0]+'"'+(cs===p[0]?' selected':'')+'>'+p[1]+'</option>';}).join('')+'</select>');}).join('')
        +row('Materials','<span class="wseg"><button data-t="standard" class="'+(u.tier==='standard'?'on':'')+'">Standard</button><button data-t="premium" class="'+(u.tier==='premium'?'on':'')+'">Premium</button></span>')
        +row('Name','<input class="ifuc" id="bn" type="text" value="'+(u.name||'').replace(/"/g,'&quot;')+'" placeholder="e.g. His · double" style="width:150px;text-align:left">')
        +'<div class="ifareas">Cut list <b>'+e.parts+' parts</b> · '+e.area+' m² board · '+e.hinges+' hinges · '+e.runners+' runner sets'+(e.rails?' · '+e.rails+' rails':'')+'</div>'
        +'<table class="iftab"><tr><td>Board (incl. 15% nesting waste)</td><td>£'+e.board+'</td></tr>'
        +'<tr><td>Hardware</td><td>£'+e.hw+'</td></tr>'
        +'<tr><td>Joiner · '+e.days+' day'+(e.days===1?'':'s')+'</td><td>£'+e.labour+'</td></tr>'
        +'<tr class="iftot"><td>Unit estimate</td><td>£'+e.price+'</td></tr></table>'
        +'<div class="ifrow"><button class="ifpdf" id="bmk">'+(edit?'Save unit':'Create unit — add to palette')+'</button></div></div></div>';
      ov.querySelector('.ifx').onclick=function(){ov.remove();};
      ov.querySelector('#bw').onchange=function(){u.wmm=Math.max(300,+this.value||1200);draw();};
      ov.querySelector('#bh').onchange=function(){u.hmm=Math.max(600,+this.value||2200);draw();};
      ov.querySelector('#bd').onchange=function(){u.dmm=Math.max(300,+this.value||600);draw();};
      ov.querySelector('#bn').onchange=function(){u.name=this.value;};
      ov.querySelectorAll('.bctr button').forEach(function(b){b.onclick=function(){u.cols=Math.max(1,Math.min(6,u.cols+(+b.getAttribute('data-d'))));draw();};});
      ov.querySelectorAll('[data-bay]').forEach(function(s){s.onchange=function(){u.colspec[+s.getAttribute('data-bay')]=s.value;draw();};});
      ov.querySelectorAll('.wseg button').forEach(function(b){b.onclick=function(){u.tier=b.getAttribute('data-t');draw();};});
      ov.querySelector('#bmk').onclick=function(){
        var e2=wardEngine(u);u.price=e2.price;u.parts=e2.parts;u.area=e2.area;
        u.base=u.cols>=3?'wslide':u.cols===2?'wdouble':'wstd';
        u.id=u.id||('cu'+Date.now().toString(36));
        if(!u.name)u.name=u.cols+'-bay '+u.wmm;
        if(!edit)custUnits.push(u);custSave();fill();ov.remove();
      };
    }
    draw();ov.addEventListener('click',function(e){if(e.target===ov)ov.remove();});
  ...
  function fill(){grid.innerHTML='';
    if(curCat==='ward'){
      var b=document.createElement('div');b.className='fpi fpbuild';b.title='Build a unit with the estimator';
      b.innerHTML='<div class="fpsym" style="display:grid;place-items:center;font-size:26px;color:#B98F16">+</div><span>Build unit</span>';
      b.onclick=function(){openBuilder();};grid.appendChild(b);
      custUnits.forEach(function(u){wire([u.base,u.name,u.wmm/10,u.dmm/10,'ward'],u);});
    }
    FURN.filter(function(f){return f[4]===curCat;}).forEach(function(f){wire(f);});}
  function wire(f,cu){
    var c=document.createElement('div');c.className='fpi'+(cu?' fpcust':'');c.title=cu?(f[1]+' \u00b7 \u00a3'+cu.price+' \u00b7 right-click to remove'):f[1];
    c.innerHTML='<div class="fpsym">'+furnSVG(f[0],f[2],f[3])+'</div><span>'+f[1]+(cu?' \u00b7 \u00a3'+cu.price:'')+'</span>';
    if(cu)c.addEventListener('contextmenu',function(e){e.preventDefault();custUnits=custUnits.filter(function(q){return q.id!==cu.id;});custSave();fill();});
    c.addEventListener('pointerdown',function(e){
      ...
      function up(ev){document.removeEventListener('pointermove',mv);document.removeEventListener('pointerup',up);ghost.remove();
        ...
        var nu={type:'furn',furn:f[0],name:f[1],cat:f[4],x:wx-f[2]/2,y:wy-f[3]/2,w:f[2],h:f[3]};
        if(cu)nu.spec={id:cu.id,price:cu.price,cols:cu.cols,parts:cu.parts,area:cu.area,tier:cu.tier};
        wardSnap(nu);add(nu);
      }
      document.addEventListener('pointermove',mv);document.addEventListener('pointerup',up);
    });
...
function wardSnap(nu){
  if(nu.cat!=='ward')return;
  var T=14/view.z,bx=nu.x,by=nu.y;
  items.forEach(function(q){
    if(q.type==='furn'&&q.cat==='ward'){
      var oy=Math.max(nu.y,q.y)<Math.min(nu.y+nu.h,q.y+q.h);
      if(oy){if(Math.abs(nu.x-(q.x+q.w))<T)bx=q.x+q.w;else if(Math.abs((nu.x+nu.w)-q.x)<T)bx=q.x-nu.w;}
      var ox=Math.max(nu.x,q.x)<Math.min(nu.x+nu.w,q.x+q.w);
      if(Math.abs(nu.y-q.y)<T&&(Math.abs(nu.x-(q.x+q.w))<T||Math.abs((nu.x+nu.w)-q.x)<T))by=q.y;
    }
    if(q.type==='plan'&&q.verts&&nu.x>q.x-T&&nu.x<q.x+q.w+T&&nu.y>q.y-T&&nu.y<q.y+q.h+T){
      if(Math.abs(nu.y-q.y)<T*2)by=q.y;
      if(Math.abs(nu.x-q.x)<T*2)bx=q.x;
      if(Math.abs((nu.x+nu.w)-(q.x+q.w))<T*2)bx=q.x+q.w-nu.w;
      if(Math.abs((nu.y+nu.h)-(q.y+q.h))<T*2)by=q.y+q.h-nu.h;
    }
  });
  nu.x=bx;nu.y=by;
}
...
var EPRICE={standard:{wallPaint:28,woodPaint:18,wallpaper:35,hardFloor:25,underlay:4,carpet:18,gripper:5.5,decDay:220,fitDay:180,joinDay:260},
            premium:{wallPaint:55,woodPaint:32,wallpaper:80,hardFloor:55,underlay:6,carpet:40,gripper:7,decDay:280,fitDay:220,joinDay:320}};
/* per-unit planning allowances for the fitted-wardrobe palette (FURN kinds, cat 'ward') */
var WPRICE={wsingle:[380,650],wstd:[420,720],wdouble:[680,1150],wwide:[780,1350],wcorner:[850,1450],wslide:[1400,2400],
            wdrawer:[180,310],wshelf:[90,150],wrail:[35,60],wshoe:[70,120]};
function wardUnits(pl){
  var all=items.filter(function(q){return q.type==='furn'&&q.cat==='ward';});
  var inPlan=all.filter(function(q){return q.x+q.w>pl.x&&q.x<pl.x+pl.w&&q.y+q.h>pl.y&&q.y<pl.y+pl.h;});
  return inPlan.length?inPlan:all;
}
function wardCalc(pl,o){
  var us=wardUnits(pl),counts={},custom=[];
  us.forEach(function(q){
    if(q.spec&&q.spec.parts)custom.push(q);
    else if(!q.spec)counts[q.furn]=(counts[q.furn]||0)+1;});
  var ti=o.tier==='premium'?1:0,lines=[],tot=0,carc=0;
  custom.forEach(function(q){
    lines.push({k:'cu_'+q.spec.id,label:(q.name||'Built unit')+' · engine · '+q.spec.parts+' parts · '+q.spec.area+'m²',qty:1,unit:'unit',u:q.spec.price,cost:q.spec.price});
    tot+=q.spec.price;}); /* engine price already carries board+hardware+labour */
  FURN.forEach(function(f){
    var n=counts[f[0]];if(!n||!WPRICE[f[0]])return;
    var u=(o.tier==='custom'&&o.custom['wd_'+f[0]]!=null)?o.custom['wd_'+f[0]]:WPRICE[f[0]][ti];
    lines.push({k:'wd_'+f[0],label:f[1],qty:n,unit:'unit',u:u,cost:n*u});tot+=n*u;
    if(['wsingle','wstd','wdouble','wwide','wcorner','wslide'].indexOf(f[0])>=0)carc+=n*(f[0]==='wslide'?2:1);
  });
  var days=carc?Math.max(0.5,Math.round(carc*0.5*2)/2):0;
  if(days){var jd=(o.tier==='custom'&&o.custom.joinDay!=null)?o.custom.joinDay:EPRICE[ti?'premium':'standard'].joinDay;
    lines.push({k:'joinDay',label:'Joiner · install · '+days+' day'+(days===1?'':'s'),qty:days,unit:'day',u:jd,cost:days*jd});tot+=days*jd;}
  return {lines:lines,total:tot,days:days,count:us.length};
}
...
function estCalc(pl,o){
  ...
  if(o.scope.ward){var wd=wardCalc(pl,o);wd.lines.forEach(function(l){lines.push(l);});}
  ...
  var tot=0;lines.forEach(function(l){if(l.cost==null)l.cost=l.qty*l.u;tot+=l.cost;});
...
function openInfo(pl){
  var o={age:'mid',cond:'good',mode:'refresh',canW:5,canD:1,cont:10,
    ...
    scope:{walls:true,ceil:true,wood:true,floor:true,ward:false},zoneTypes:{}};
  ...
  ov.innerHTML='<div class="trbox infobox"><div class="ifhd"><span class="oh">'+(pl.name||'Floor plan')+' \u00b7 room information</span><button class="ifx">\u00d7</button></div>'
    +'<div class="iftabs"><button class="ift on" data-t="paint">Paint</button><button class="ift" data-t="paper">Wallpaper</button><button class="ift" data-t="ward">Wardrobes</button><button class="ift" data-t="est">Estimate</button><button class="ift" data-t="ward">Wardrobes</button><button class="ift" data-t="guides">Guides</button></div>'
  ...
  function estTab(){
    ...
    var h='<div class="ifrow"><span>Room</span><select class="ifsel" id="ifroom">'+[['kitchen','Kitchen'],['utility','Utility'],['living','Living room'],['dining','Dining room'],['bed','Bedroom'],['office','Home office'],['bath','Bathroom'],['ensuite','Ensuite'],['hall','Hall + landing']].map(function(q){return '<option value="'+q[0]+'"'+(o.room===q[0]?' selected':'')+'>'+q[1]+'</option>';}).join('')+'</select></div>'
      +'<div class="ifrow"><span>Decorating</span><span class="ifchks">'+chk('walls','Walls',o.scope.walls)+chk('ceil','Ceilings',o.scope.ceil)+chk('wood','Woodwork',o.scope.wood)+chk('floor','Flooring',o.scope.floor)+chk('ward','Wardrobes',o.scope.ward)+'</span></div>';
  ...
  function wardTab(){
    var w=wardCalc(pl,o);
    if(!w.count)return '<div class="ifareas" style="padding:26px 4px">No fitted-wardrobe units on this plan yet \u2014 drag units, drawer packs and internals from the furniture palette (Fitted wardrobes) onto the room, then come back for the estimate.</div>';
    var h='<div class="ifrow"><span>Materials</span>'+seg('tier',[['standard','Standard'],['premium','Premium'],['custom','Custom \u00a3']],o.tier)+'</div>'
      +'<div class="ifareas">'+w.count+' unit'+(w.count===1?'':'s')+' on the plan \u00b7 install allowance <b>'+w.days+' day'+(w.days===1?'':'s')+'</b></div>'
      +'<table class="iftab estab"><tr><th>Item</th><th>Qty</th><th>Unit \u00a3</th><th>\u00a3</th></tr>';
    w.lines.forEach(function(l){
      h+='<tr><td>'+l.label+'</td><td>'+l.qty+(l.unit==='day'?'':' \u00d7')+'</td><td>'
        +(o.tier==='custom'?'<input class="ifuc" data-uk="'+l.k+'" type="number" step="1" value="'+l.u+'">':('\u00a3'+l.u))
        +'</td><td>\u00a3'+Math.round(l.cost)+'</td></tr>';});
    h+='<tr class="iftot"><td>Wardrobe budget</td><td></td><td></td><td>\u00a3'+Math.round(w.total)+'</td></tr></table>'
      +'<div class="ifareas">Tick Wardrobes on the Estimate tab to roll this into the room budget \u00b7 allowances, not quotes</div>';
    return h;
  }
  function guidesTab(){
  ...
  function draw(tab){
    ...
    body.querySelectorAll('.ifuc').forEach(function(inp){inp.addEventListener('change',function(){var uk=inp.getAttribute('data-uk');o.custom[uk]=+inp.value||0;draw();});});
...
/* tilt-cards — Motion+ TiltCard port: swatch & photo media tilt toward the pointer (drag unaffected) */
(function(){
var MAX=9;
function mediaOf(c){return c.querySelector('.fill')||c.querySelector('.inner img');}
document.addEventListener('pointermove',function(e){
  var c=e.target.closest?e.target.closest('.citem.swatch,.citem.img'):null;
  if(!c)return;
  var m=mediaOf(c);if(!m)return;
  if(e.buttons){m.style.transform='';return;} /* dragging: hands off */
  var r=c.getBoundingClientRect();
  var rx=MAX*(.5-(e.clientY-r.top)/r.height),ry=MAX*((e.clientX-r.left)/r.width-.5);
  m.style.transition='transform .25s cubic-bezier(.22,1,.36,1)';
  m.style.transform='perspective(500px) rotateX('+rx.toFixed(2)+'deg) rotateY('+ry.toFixed(2)+'deg)';
},{passive:true});
document.addEventListener('pointerout',function(e){
  var c=e.target.closest?e.target.closest('.citem.swatch,.citem.img'):null;
  if(!c)return;
  if(e.relatedTarget&&c.contains(e.relatedTarget))return;
  var m=mediaOf(c);if(m){m.style.transition='transform .4s cubic-bezier(.22,1,.36,1)';m.style.transform='';}
},{passive:true});
})();
