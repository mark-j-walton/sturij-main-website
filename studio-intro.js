...
(function(){
  ...
  /* ---------- the real pairing sets this intro showcases ----------
     The list lives in showcase/pairings/sets.json (regenerated by
     scripts/build-pairings.mjs), so dropping a new exported set in creates
     fresh content with no code change. Up to SHOW sets are displayed —
     picked at random when more are available, so each landing can differ. */
  var SHOW=3;
  ...
  var FALLBACK_SETS=[1,2,3].map(function(n){return {dir:'showcase/pairings/set-'+n+'/',render:'render.jpg',panels:PANELS};});
  function pickSets(list){
    var a=list.slice();
    for(var i=a.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),t=a[i];a[i]=a[j];a[j]=t;}
    return a.slice(0,SHOW).map(function(d){
      return {dir:d.dir.replace(/\/?$/,'/'),render:d.render||'render.jpg',panels:d.panels||PANELS};
    });
  }
  function loadSets(cb){
    if(OVR.sets)return cb(pickSets(OVR.sets));
    try{
      fetch(assetURL('showcase/pairings/sets.json')).then(function(r){return r.json();})
        .then(function(list){cb(pickSets(list));})
        .catch(function(){cb(pickSets(FALLBACK_SETS));});
    }catch(e){cb(pickSets(FALLBACK_SETS));}
  }

  var GOLD=0xD4A01B,NAVY=0x1D1D1D;
  var scene,camera,renderer,root,chips=[],sparkles,goldLight,raf=0,t0=0,leaving=false,leaveT=0,panAmp=2.2;
  ...
  function realise(mat,p){
    ...
    im.onload=function(){
      ...
      if(reduced&&renderer)renderer.render(scene,camera);
    };
  ...
  function build(){
    ...
    buildSparkles();
    layer.classList.add('has3d');            /* hides the static fallback collage */
    loadSets(function(sets){
      buildGallery(sets);fitPortrait();
      if(reduced&&renderer)renderer.render(scene,camera);
    });
  }
  /* portrait phones: lift the gallery into the upper half, clear of the text */
  function fitPortrait(){if(root)root.position.y=layer.clientHeight>layer.clientWidth?-.35:-.55;}
  ...
  function buildGallery(SETS){
    ...
    for(var i=0;i<SETS.length;i++){
      ...
      var cx=off*4.5,cz=-3.1-Math.abs(off)*1.2,ry=-off*.17; /* shallow arc facing the camera */
      var H=3.15,W=3.15;                                    /* renders are square */

      /* frame + canvas */
      var frame=new THREE.Mesh(new THREE.BoxGeometry(W+.18,H+.18,.1),frameMat);
      frame.position.set(cx,2.25,cz);frame.rotation.y=ry;frame.castShadow=true;g.add(frame);
      ...
      cv.position.set(cx+Math.sin(ry)*.06,2.25,cz+Math.cos(ry)*.06);
    ...
    panAmp=Math.max(1.6,((SETS.length-1)/2)*4.5*.5);
  ...
  var go=document.getElementById('i3go');
